    ...
    "show the questionnaire view" in {
      showQuestionnaireWithUnauthorisedUser {
        result =>
          ...
          document.title() must be("You’ve signed out - Payrolling benefits and expenses - GOV.UK")
    ...
    "show the questionnaire thank you view" in {
      thankYouWithUnauthorisedUser {
        result =>
          status(result) must be(OK)
          val document = Jsoup.parse(contentAsString(result))
          document.title() must be("Thanks for your feedback - Payrolling benefits and expenses - GOV.UK")
