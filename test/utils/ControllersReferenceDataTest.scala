...
class ControllersReferenceDataTest extends PlaySpec with OneAppPerSuite with FakePBIKApplication
  ...
  object StubPbikConfig extends AppConfig with ServicesConfig {
      ...
      private val contactHost = Play.current.configuration.getString("microservice.services.contact-frontend.host").
      ...
      override lazy val cyEnabled = Play.current.configuration.getBoolean("pbik.enabled.cy").getOrElse(false)
