...
class BenefitTypesSpec extends FakePBIKApplication {
  ...
  "BenefitTypes" should {
    ...
    "fail to deserialize when key is wrong" in {
      val invalidJson =
        """
          |{
          |  "IabdTypeTT": []
      ...
      val result = Json.fromJson[BenefitTypes](Json.parse(invalidJson))
    ...
    "fail to deserialize when JSON is empty" in {
      ...
      val result = Json.fromJson[BenefitTypes](emptyJson)
      result.isError mustBe true
    }
  }
}